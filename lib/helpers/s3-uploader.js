 ...
 */
	static async uploadContentS3Path(buckets, contentS3Path, body) {
		...
		const defaultSuccess = await this.uploadToBucket(defaultBucket, contentS3Path, body);

		if(defaultSuccess)
			return;

		const provisionalSuccess = await this.uploadToBucket(provisionalBucket, contentS3Path, body);

		if(provisionalSuccess)
			return;

		throw new SqsEmitterError('Failed to upload to both default and provisional buckets', SqsEmitterError.codes.S3_ERROR);
	}
