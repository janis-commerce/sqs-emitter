...
class ParameterStore {
	...
	static async getParameterValue() {
		...
		try {
			...
			throw new Error(`Unable to get parameter with arn ${parameterArn} - ${error.message}`);
		}
	}
	...
	static async getParameterArnFromRAM() {
		...
		try {
			...
			if(!filteredResources.length)
				logger.error(`Unable to find resources with parameter ${this.parameterName} in the ARN`);

			return filteredResources[0].arn;

		} catch(error) {
			throw new Error(`Error listing ram resources with parameter name: ${this.parameterName}`);
		}
	}
