...
module.exports = class SqsEmitter {
	...
	async publishEvent(sqsUrl, event) {
		...
		if(parsedEvent.limitExceeded) {
			...
			const { formattedEvent, s3UploadRequest } = await this.formatAndUploadEventWithS3Content(parsedEvent);
	...
	async formatAndUploadEventWithS3Content(parsedEvent) {
		...
		const sqsFixedContent = this.formatBodyWithContentS3Path(parsedEventBase, contentS3Path, payloadFixedProperties);

		return {
			formattedEvent: { ...parsedEventBase, MessageBody: JSON.stringify(sqsFixedContent) },
