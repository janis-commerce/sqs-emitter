...
module.exports = class SqsEmitter {
	...
	async publishEvent(sqsUrl, event) {
		...
		if(parsedEvent.limitExceeded) {
			...
			const { formattedEvent, s3UploadRequest } = await this.formatAndUploadEventWithS3Content(parsedEventBase, extraProperties);
	...
	async publishEvents(sqsUrl, events) {
		...
		const asyncWithConcurrency = new AsyncWithConcurrency(async batch => {
			...
			for(const { limitExceeded, extraProperties, ...parsedEvent } of batch) {
				...
				const { formattedEvent, s3UploadRequest } = await this.formatAndUploadEventWithS3Content(parsedEvent, extraProperties);

				formattedSqsBatch.push(formattedEvent);

				promises.push(s3UploadRequest);
			}
	...
	async formatAndUploadEventWithS3Content(parsedEvent, extraProperties) {

		const { contentS3Path, payloadFixedProperties } = extraProperties;
		...
		return {
			formattedEvent: { ...parsedEvent, MessageBody: JSON.stringify(sqsFixedContent) },
			s3UploadRequest: S3Uploader.uploadContentS3Path(bucketList, contentS3Path, parsedEvent.MessageBody)
		};
	}
