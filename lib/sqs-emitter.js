...
module.exports = class SqsEmitter {
	...
	constructor() {
		/** @private */
		this.sqs ??= captureAWSv3Client(new SQSClient());
	}
	...
	async publishEvent(sqsUrl, event) {
		...
		if(parsedEventSize > SQS_MESSAGE_LIMIT_SIZE) {
			...
			if(formattedEvent.Error)
				return this.formatSingleSQSResponse({ failed: [formattedEvent.Error] });
		...
		try {
			...
			const { MessageId, SequenceNumber } = await this.sqs.send(new SendMessageCommand({
				...formattedSqsMessage,
				QueueUrl: sqsUrl
			}));

			return this.formatSingleSQSResponse({
				success: [{
					messageId: MessageId,
			...
			return this.formatSingleSQSResponse({
				failed: [{
					...
					Code: SqsEmitterError.codes.SEND_SQS_MESSAGE_ERROR
				}]
			});
		}
	}

	formatSingleSQSResponse({ success = [], failed = [] }) {
		return {
	...
	async publishEvents(sqsUrl, events) {
		...
		return this.formatBatchSQSResponse(results);
	}

	formatBatchSQSResponse(results) {
	...
	async formatAndUploadEventWithS3Content(parsedEvent) {
		...
		if(!bucketInfo) {
			return {
				Error: {
					...parsedEvent.Id && { Id: parsedEvent.Id },
