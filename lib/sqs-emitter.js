...
module.exports = class SqsEmitter {
	...
	isValidSqsUrl(url) {
		const standardQueuePattern = /^https:\/\/sqs\.[a-zA-Z0-9-]+\.amazonaws\.com\/[0-9]{12}\/[a-zA-Z0-9\-_]+$/;
		const fifoQueuePattern = /^https:\/\/sqs\.[a-zA-Z0-9-]+\.amazonaws\.com\/[0-9]{12}\/[a-zA-Z0-9\-_]+\.fifo$/;
		return standardQueuePattern.test(url) || fifoQueuePattern.test(url);
	}
};
