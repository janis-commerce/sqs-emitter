...
module.exports = class SqsEmitter {
	...
	async publishEvent(sqsUrl, event) {
		...
		if(parsedEvent.limitExceeded) {
			...
			const sqsFixedContent = this.formatBodyWithS3ContentPath(parsedEvent, contentS3Path, payloadFixedProperties);
			...
			await S3Uploader.uploadS3ContentPath(bucketList, contentS3Path, parsedEvent.MessageBody);
		}
	...
	async publishEvents(sqsUrl, events) {
		...
		const asyncWithConcurrency = new AsyncWithConcurrency(async batch => {
			...
			for(const { limitExceeded, extraProperties: { payloadFixedProperties, contentS3Path }, ...parsedEvent } of batch) {
				...
				promises.push(S3Uploader.uploadS3ContentPath(bucketList, contentS3Path, parsedEvent.MessageBody));
			}
	...
	formatBodyWithS3ContentPath(parsedEvent, contentS3Path, payloadFixedProperties) {
		return {
			contentS3Path,
	...
	handleEventSizeLimit(parsedEvent, parsedEventSize) {
		...
		if(parsedEventSize > SQS_MESSAGE_LIMIT_SIZE) {
			...
			const contentFixed = this.formatBodyWithS3ContentPath(parsedEvent, contentS3Path, payloadFixedProperties);
	...
	parseExtraProperties(event, sqsName) {
		...
		return {
			...
			contentS3Path: this.getS3ContentPath(sqsName)
		};
	}
